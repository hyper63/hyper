...
import { fork, isFile, isMultipartFormData, isTrue } from './utils.ts'
...
Deno.test('utils', async (t) => {
  await t.step('fork', async (t) => {
    ...
    await t.step(
      'should set the status and response correctly',
      async () => {},
    )
  ...
  await t.step('isFile', async (t) => {
    await t.step('should return true if path points to a file', () => {
  ...
  await t.step('isTrue', async (t) => {
    await t.step('should return true if value is true-like', () => {
      assert(isTrue(true))
      assert(isTrue('true'))
      assert(!isTrue('t'))
      assert(!isTrue(false))
