...
import { opine, json, R, cors, helmet, MultipartReader, exists } from './deps.js'
...
export default function (services) {
  ...
  const upload = (fieldName = 'file') => async (req, _res, next) => {
    let boundary
  ...
  app.put('/data/:db/:id', json({ limit: '8mb' }), bindCore, data.updateDocument)
